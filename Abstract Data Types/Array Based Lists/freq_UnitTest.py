import unittest
from Task_6 import TextEditor
from Task_6 import bubble_sort
...
class myTest(unittest.TestCase):
    ...
    def test_frequency(self):
        ...
        filename = 'freq.txt'
        obj.read_filename(filename)
        self.assertTrue(obj.frequency())


    def test_bubble_sort(self):
        L = ['Hello', 'World', 'Hello', 'Bye']
        sorted = bubble_sort(L)
        self.assertEqual(sorted, ['Bye', 'Hello', 'Hello', 'World'])            # Case_1: Filled list being sorted correctly
        empty = []
        sorted1 = bubble_sort(empty)
        self.assertEqual(sorted1, [])                                           # Case_2: Empty list being sorted back to empty list
