import Task_6
from Task_6 import ArrayOperations
...
class TextEditor:
    ...
    def insert_num(self, num, line):
        ...
        try:
            ...
            self.stack.push(('delete', num, None))                         # when user inserts, do the reverse action and push delete onto the stack, don't need to input any text so None
        except ValueError:
            return False
        except IndexError:
            return False
    ...
    def delete_num(self, num):
        ...
        try:
            if num == '':
                self.stack.push(['insert', None, self.array])                          # if he uses empty line number, then we insert position as None & the whole array onto the stack
                self.array = ArrayOperations()
            else:
                num = int(num)
                self.array.delete(num)
                self.stack.push(('insert', num, line))
        except IndexError:
            return False
        except ValueError:
            return False
    ...
    def undo(self):
        '''
        This function undo's the operations depending on whether the user inserts/deletes a line/lines
        :param: None
        :precondition: The user must hae inserted or deleted something already
        :postcondition: The user's action is gone undone and gone to its previous state
        :complexity: Best Case = Worst Case = O(1), constant time complexity
        '''

        operation = self.stack.pop()
        if operation[0] == 'delete':                                                #if user inserts, we find delete in the stack and delete the line of text he entered at index 1
            self.array.delete(operation[1])

        elif operation[0] == 'insert':                                              # if user deletes, we find insert in the stack and assign the array to the lines he deleted
            if operation[1] == None:
                self.array = operation[2]

            elif operation[1] != None:                                              # if user deletes with a position number, we insert the line number back at the position
                self.array.insert(operation[1], operation[2])
...
class Stack:
    ...
    def __init__(self):
        self.stack_array = ArrayOperations()

    def size(self):
        '''
        This function returns the length of the stack i.e. number of items in the stack
        :param: None
        :precondition: None
        :postcondition: The length of the stack is returned
    ...
    def is_empty(self):
        '''
        This function returns the length of the stack when its empty i.e. no elements in the stack
        :param: None
        :precondition: None
        :postcondition: None
        :oomplexity: Best Case = Worst Case = O(1), constant time complexity.
    ...
    def push(self, item):
        ...
        :precondition: None
        :postcondition: The updated stack with the new item pushed onto it
        :oomplexity: Best Case = Worst Case = O(1), constant time complexity.
        '''
    ...
    def pop(self):
        '''
        This function pops/removes the last item pushed onto the stack
        :param: None
        :precondition: None
        :postcondition: The stack is updated after the item is popped
        :oomplexity: Best Case = Worst Case = O(1), constant time complexity.
        '''

        if not self.is_empty():
            x = self.stack_array[(self.size())]                                     # locating the item
            self.stack_array.delete(self.size())                                    # deleting the item
            return x                                                                # returning it to the main texteditor
        else:
