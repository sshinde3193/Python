class ArrayOperations:
    ...
    def __eq__(self, other):
        ...
        result = False
        if len(self) == len(other):                                         # handling for the length of both lists
            for i in range(self.count):
    ...
    def append(self, item):
        ...
        space_left = not self.is_full()
        if space_left:
            self.array[self.count] = item
            self.count += 1
        else:
            raise Exception('The list is full.')                                # Accounting for when list is full
        ...
        while result == False:
            for i in range(self.count):
                if item == self.array[i]:                                               # Linear search and then calling the delete function
                    ...
                    result = False
            if result == False:
                raise ValueError                                                        # if item not found raises ValueError
